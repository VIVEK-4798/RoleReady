...
const VenueTable = ({ searchParameter = "", refresh }) => {
 ...
 const fetchVenues = async () => {
  ...
  try {
    ...
    const userId = localStorage.getItem("mentor-userId") || localStorage.getItem("adminId");
    
    const endpoint = role === "mentor" 
      ? `${api}/api/venue/get-mentor-venues`
      : `${api}/api/venue/get-venue`;

    const headers = role === "mentor"
      ? { user_id: userId }
      : { id: userId }; // Existing admin usage
