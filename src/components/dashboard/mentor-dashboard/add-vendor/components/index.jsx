...
const Index = () => {
  ...
  const validateFormData = (formData) => {
    ...
    const getUserId = () => {
      const role = localStorage.getItem("role");
      if (role === "admin") return localStorage.getItem("adminId");
      if (role === "mentor") return localStorage.getItem("mentor-userId");
      return null;
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentError.length > 0) {
      ...
      ApiBody = {
        ...ApiBody,
        created_by_user_id: getUserId(), // used for tracking job owner
      };
    }else if (mode === "edit") {
        if (ApiBody.name) {
          delete ApiBody.name;
      ...
      try {
        ...
        if (response.data.results === true) {
          ...
          navigate("/mentor-dashboard/vendors");
        } else {
